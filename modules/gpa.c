...
{
double full_marks = result.course->credit * 100.0; if (full_marks <= 0.0) return 0.0;
...
{
double weighted_points = 0.0; double total_credits = 0.0;
for (int i = 0; i < n_results; i++)
{
weighted_points += getGradePoint(results[i]) * results[i].course->credit; total_credits += results[i].course->credit;
}

if (total_credits == 0.0) return 0.0; return weighted_points / total_credits;
}
